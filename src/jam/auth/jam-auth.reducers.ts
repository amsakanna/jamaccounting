...
export function authReducers ( state = initialState, action: AuthAction.All ): AuthState
{
	switch ( action.type ) {
		case AuthActionTypes.initialize: return { ...state, initialized: false, loading: true };
		...
		case AuthActionTypes.deauthenticated: return { ...state, authenticated: false };
		case AuthActionTypes.setUser: return { ...state, user: action.user };
		case AuthActionTypes.requestRegisterPage: return { ...state };
		...
		case AuthActionTypes.requestSignInPage: return { ...state };
